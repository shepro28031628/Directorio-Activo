...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Cambiar estado en la API de Microsoft Graph
    ...
    try {
      ...
      if (tenantId && clientId && clientSecret) {
        ...
        if (tokenResp.ok) {
          ...
          // Encontrar usuario en Azure por su UPN (correo) o ID
          ...
          if (userLookup.ok) {
            ...
            if (updateResp.ok) {
              ...
              // Si se suspende, revocar también las sesiones activas
              if (suspended) {
    ...
    await prisma.auditoria.create({
      data: {
        ...
        detalles: `${suspended ? 'Deshabilitada' : 'Reactivada/Vinculada'} la cuenta de Microsoft 365 para ${mUser.nombre} (${mUser.correo}). Realizado vía API: ${apiSuccess ? 'Sí' : 'Simulada'}.`,
