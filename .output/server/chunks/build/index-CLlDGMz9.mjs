...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-b border-white/10 pb-5"><div><div class="flex flex-wrap items-center gap-3"><h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight text-white font-sans"> Active Directory <span class="gradient-text-cyan">Nodos &amp; Servicios</span></h1><span class="px-3 py-1 rounded-full text-xs font-mono font-bold bg-neon-cyan/10 text-neon-cyan border border-neon-cyan/30 flex items-center gap-1.5 shadow-glow-cyan"><span class="w-2 h-2 rounded-full bg-neon-cyan animate-ping"></span> Bosque Active Directory (renova.local) </span></div><p class="text-slate-400 text-xs sm:text-sm mt-1"> Visualizaci\xF3n de mapa de nodos interactivo para OUs, GPOs en 3D, Servicios DNS/DHCP y Matriz RBAC. </p></div>`);
      if (adData.value) {
      ...
      if (isLoading.value) {
        ...
        if (activeTab.value === "ous") {
          _push(`<div class="space-y-6"><div class="grid grid-cols-1 sm:grid-cols-3 gap-4"><div class="bento-card p-5"><span class="text-xs font-mono font-bold text-slate-400 uppercase tracking-wider block">Total Objetos AD</span><span class="text-3xl font-extrabold text-white font-mono mt-1 block">450 Objetos</span><span class="text-xs text-neon-cyan font-mono flex items-center gap-1 mt-1"> \u2713 Bosque renova.local Sincronizado </span></div><div class="bento-card p-5"><span class="text-xs font-mono font-bold text-slate-400 uppercase tracking-wider block">Dispositivos Conectados</span><span class="text-3xl font-extrabold text-neon-emerald font-mono mt-1 block">180 Equipos</span><span class="text-xs text-slate-400 font-mono block mt-1"> 145 Workstations \u2022 25 Servidores \u2022 10 Kioskos </span></div><div class="bento-card p-5"><span class="text-xs font-mono font-bold text-slate-400 uppercase tracking-wider block">Nivel Funcional</span><span class="text-3xl font-extrabold text-neon-purple font-mono mt-1 block">Win Server 2022</span><span class="text-xs text-slate-400 font-mono block mt-1"> Active Directory Domain Services (AD DS) </span></div></div><div class="bento-card p-5 sm:p-6 space-y-6"><div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2"><div><h2 class="text-base sm:text-lg font-bold text-white font-mono flex items-center gap-2"><span>\u{1F310}</span> Mapa Interactivo de Nodos de Unidades Organizativas (OUs) </h2><p class="text-xs text-slate-400 mt-0.5">Haz clic en cualquier nodo para expandir la ficha Sci-Fi Inspector o inspeccionar el DistinguishedName (DN).</p></div><span class="px-3 py-1 rounded-full text-xs font-mono font-bold bg-neon-cyan/10 text-neon-cyan border border-neon-cyan/30"> Modo \xC1rbol T\xE1ctico 2026 </span></div><div class="p-6 bg-[#070A12] border border-white/10 rounded-2xl relative overflow-x-auto custom-scrollbar min-h-[380px] flex flex-col justify-center"><div class="flex justify-center mb-8"><div class="px-6 py-3 rounded-2xl bg-gradient-to-r from-cyan-500 to-blue-600 border border-white/30 text-white font-mono font-bold text-sm shadow-glow-cyan cursor-pointer hover:scale-105 transition-all text-center flex items-center gap-3"><span class="w-3 h-3 rounded-full bg-white animate-ping"></span> \u{1F333} REN Corporativo (DC=ren,DC=local) <span class="px-2 py-0.5 rounded-full bg-black/40 text-xs">450 Objetos</span></div></div><div class="w-full flex justify-around border-t-2 border-dashed border-cyan-500/30 pt-8 relative"><div class="flex flex-col items-center space-y-4 max-w-xs"><div class="p-4 rounded-2xl bg-slate-900 border border-neon-cyan/40 hover:border-neon-cyan text-white font-mono text-xs shadow-glow-cyan cursor-pointer transition-all text-center space-y-1"><span class="font-bold text-neon-cyan block">\u{1F4CD} Sede Bogot\xE1</span><span class="text-[10px] text-slate-400 block font-mono">OU=Sede Bogota</span><span class="inline-block px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 text-[10px] font-bold mt-1">280 Objetos</span></div><div class="space-y-2 w-full pl-4 border-l-2 border-cyan-500/20"><div class="p-2.5 rounded-xl bg-slate-950/80 border border-white/10 hover:border-neon-cyan/50 text-[11px] font-mono text-slate-300 cursor-pointer flex justify-between items-center group"><span>\u{1F4BB} TI (24 Users / 32 PCs)</span><span class="text-neon-cyan opacity-0 group-hover:opacity-100 transition-opacity">HUD \u2794</span></div><div class="p-2.5 rounded-xl bg-slate-950/80 border border-white/10 hover:border-neon-cyan/50 text-[11px] font-mono text-slate-300 cursor-pointer flex justify-between items-center group"><span>\u{1F4C8} Finanzas (45 Users / 50 PCs)</span><span class="text-neon-cyan opacity-0 group-hover:opacity-100 transition-opacity">HUD \u2794</span></div><div class="p-2.5 rounded-xl bg-slate-950/80 border border-white/10 hover:border-neon-cyan/50 text-[11px] font-mono text-slate-300 cursor-pointer flex justify-between items-center group"><span>\u{1F465} RRHH (18 Users / 20 PCs)</span><span class="text-neon-cyan opacity-0 group-hover:opacity-100 transition-opacity">HUD \u2794</span></div></div></div><div class="flex flex-col items-center space-y-4 max-w-xs"><div class="p-4 rounded-2xl bg-slate-900 border border-neon-cyan/40 hover:border-neon-cyan text-white font-mono text-xs shadow-glow-cyan cursor-pointer transition-all text-center space-y-1"><span class="font-bold text-neon-cyan block">\u{1F4CD} Sede Medell\xEDn</span><span class="text-[10px] text-slate-400 block font-mono">OU=Sede Medellin</span><span class="inline-block px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 text-[10px] font-bold mt-1">120 Objetos</span></div><div class="space-y-2 w-full pl-4 border-l-2 border-cyan-500/20"><div class="p-2.5 rounded-xl bg-slate-950/80 border border-white/10 hover:border-neon-cyan/50 text-[11px] font-mono text-slate-300 cursor-pointer flex justify-between items-center group"><span>\u{1F4E6} Operaciones (65 Users / 70 PCs)</span><span class="text-neon-cyan opacity-0 group-hover:opacity-100 transition-opacity">HUD \u2794</span></div></div></div><div class="flex flex-col items-center space-y-4 max-w-xs"><div class="p-4 rounded-2xl bg-slate-900 border border-neon-cyan/40 hover:border-neon-cyan text-white font-mono text-xs shadow-glow-cyan cursor-pointer transition-all text-center space-y-1"><span class="font-bold text-neon-cyan block">\u{1F5A5}\uFE0F Dispositivos Flota</span><span class="text-[10px] text-slate-400 block font-mono">OU=Dispositivos</span><span class="inline-block px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 text-[10px] font-bold mt-1">180 Equipos</span></div><div class="space-y-2 w-full pl-4 border-l-2 border-cyan-500/20"><div class="p-2.5 rounded-xl bg-slate-950/80 border border-white/10 hover:border-neon-cyan/50 text-[11px] font-mono text-slate-300 cursor-pointer flex justify-between items-center group"><span>\u{1F4BB} Workstations (145 Equipos)</span><span class="text-neon-cyan opacity-0 group-hover:opacity-100 transition-opacity">HUD \u2794</span></div><div class="p-2.5 rounded-xl bg-slate-950/80 border border-white/10 hover:border-neon-cyan/50 text-[11px] font-mono text-slate-300 cursor-pointer flex justify-between items-center group"><span>\u{1F5A5}\uFE0F Servidores (25 Equipos)</span><span class="text-neon-cyan opacity-0 group-hover:opacity-100 transition-opacity">HUD \u2794</span></div></div></div></div></div>`);
        ...
        if (activeTab.value === "gpos") {
          ...
          ssrRenderList(adData.value.gpos, (gpo) => {
            _push(`<div class="bento-card p-5 space-y-3 flex flex-col justify-between"><div><div class="flex justify-between items-start gap-2"><div><span class="px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold bg-neon-cyan/10 text-neon-cyan border border-neon-cyan/30">${ssrInterpolate(gpo.category)}</span><h3 class="font-bold text-white text-base mt-2 font-mono">${ssrInterpolate(gpo.name)}</h3></div><span class="${ssrRenderClass([gpo.status === "Enforced" ? "bg-amber-500/10 text-amber-400 border-amber-500/30" : "bg-neon-cyan/10 text-neon-cyan border-neon-cyan/30", "px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold border shrink-0"])}">${ssrInterpolate(gpo.status)}</span></div><p class="text-xs text-slate-400 mt-2 font-mono">${ssrInterpolate(gpo.description)}</p><div class="mt-3 p-3 rounded-xl bg-[#080C14] border border-white/10 text-xs font-mono"><div class="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">\xC1mbito: ${ssrInterpolate(gpo.scope)}</div>`);
            if (gpo.settings.drives) {
              _push(`<div class="space-y-1 mt-2 text-neon-cyan"><!--[-->`);
              ssrRenderList(gpo.settings.drives, (drive) => {
                _push(`<div> \u{1F4C2} Mapeo ${ssrInterpolate(drive.letter)} \u2794 ${ssrInterpolate(drive.path)} (${ssrInterpolate(drive.label)}) </div>`);
              });
              _push(`<!--]--></div>`);
            } else if (gpo.settings.passwordMinLength) {
              _push(`<div class="space-y-1 text-slate-300"><div>\u{1F512} M\xEDn. Contrase\xF1a: ${ssrInterpolate(gpo.settings.passwordMinLength)} Caracteres</div><div>\u{1F6E1}\uFE0F LAPS Activado: ${ssrInterpolate(gpo.settings.lapsEnabled ? "S\xED (Local Admin Pass)" : "No")}</div></div>`);
            } else {
              _push(`<!---->`);
            }
...
//# sourceMappingURL=index-CLlDGMz9.mjs.map
